...
def optimize_hyperparameters(x_train, y_train):
    """
    Optimizes ExtraTreesRegressor hyperparameters using Optuna with stratified k-fold cross validation.
    """
...
def run_analysis(x, y, suffix):
    """
    Runs the complete analysis pipeline with ExtraTreesRegressor using KFold for train-test splitting.
    ...
    if fold_results:
        ...
        plot_predictions(
            ...
            f'Training set predictions from ET model ({suffix})',
            f'ET_train_predictions_{suffix}_fold{closest_fold_num}.tif'
        )
        plot_predictions(
            closest_fold['y_test'], closest_fold['y_test_pred'],
            f'Test set predictions from ET model ({suffix})',
            f'ET_test_predictions_{suffix}_fold{closest_fold_num}.tif'
        )
    else:
...
if __name__ == "__main__":
    ...
    with open('ET_model_results_comparison.json', 'w') as f:
    ...
    plt.tight_layout()
    plt.savefig('ET_metrics_comparison.png', dpi=300, bbox_inches='tight')
    plt.close()
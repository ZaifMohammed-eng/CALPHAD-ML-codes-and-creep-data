...
def optimize_hyperparameters(x_train, y_train):
    """
    Optimizes XGBoost hyperparameters using Optuna with stratified k-fold cross validation.
    """
    x_train_array = x_train.values if isinstance(x_train, pd.DataFrame) else x_train
    y_train_array = y_train.to_numpy() if isinstance(y_train, pd.Series) else y_train

    unique_y = len(np.unique(y_train_array))
    n_bins = min(10, unique_y)
    print(f"Number of unique target values: {unique_y}, using {n_bins} bins")

    try:
        y_binned = pd.qcut(y_train_array.ravel(), q=n_bins, labels=False, duplicates='drop')
    except ValueError as e:
        print(f"Warning: qcut failed with error {str(e)}. Using pd.cut instead.")
...
def run_analysis(x, y, suffix):
    """
    Runs the complete analysis pipeline with XGBoost Regressor using KFold.
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(x), 1):
        ...
        print(f"Best hyperparameters for Fold {fold}: {best_params}")
        ...
        fold_pipeline = Pipeline([
            ('preprocessor', create_preprocessing_pipeline()),
            ('xgb', xgb.XGBRegressor(**best_params, n_jobs=-1, random_state=random_seed))
        ])
    ...
    if fold_results:
        ...
        plot_predictions(
            ...
            f'Training set predictions from XGB model ({suffix})',
            f'XGB_train_predictions_{suffix}_fold{closest_fold_num}.tif'
        )
        plot_predictions(
            closest_fold['y_test'], closest_fold['y_test_pred'],
            f'Test set predictions from XGB model ({suffix})',
            f'XGB_test_predictions_{suffix}_fold{closest_fold_num}.tif'
        )
    else:
    ...
    final_pipeline = Pipeline([
        ('preprocessor', create_preprocessing_pipeline()),
        ('xgb', xgb.XGBRegressor(**first_fold_best_params, n_jobs=-1, random_state=random_seed))
    ])
...
if __name__ == "__main__":
    ...
    if not os.path.exists('ML_data_creep_project.csv'):
        raise FileNotFoundError("Ensure 'ML_data_creep_project.csv' is in the working directory.")
    ...
    print(f"Selected columns for x_new: {Data.columns[selected_cols].tolist()}")
    ...
    with open('XGB_model_results_comparison.json', 'w') as f:
